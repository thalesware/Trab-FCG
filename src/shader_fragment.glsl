...
void main()
{
    ...
    if ( object_id == SPHERE )
    {
        ...
        float ro = 0.1f;
        vec4 p_planeta = bbox_center + ro*(normalize(position_model - bbox_center));
        vec4 p_vec = p_planeta - bbox_center;

        float teta = atan(p_vec.x, p_vec.z);
        float fi = asin(p_vec.y/ro);
